    ...
    public class ComponentDataUtil
    {
        ...
        public List<Component> GetComponents(List<string> keywords)
        {
            ...
            try
            {
                ...
                foreach (var comp in GetAllComponents())
                {
                    ...
                    //string reservation = Convert.ToString(comp.ResarvationID);

                    for (int i = 0; i < keywords.Count; i++)
                    {
                        if (c > 1 && !keywords.Contains("+"))
                        {
                            if (compCur.ComponentInfo.Contains(key[i]) || owner.Contains(key[i]) ||
                        ...
                        if (keywords.Contains("+"))
                        {
                            ...
                            if (i < nkey.Count - 1)
                            {
                                if (compCur.ComponentInfo.Contains(nkey[i]) &&
                                    compCur.ComponentInfo.Contains(nkey[i + 1]) || owner.Contains(nkey[i]) ||
                                    compCur.ComponentName.Contains(nkey[i]) &&
                                    compCur.ComponentName.Contains(nkey[i + 1]) ||
                ...
                using (rdr = oCmd.ExecuteReader())
                {
                    while (rdr.Read())
                    {
                        ...
                        if (rdr["ReservationID"] != DBNull.Value)
                            newComp.ReservationID = (int)rdr["ReservationID"];
        ...
        public bool UpdateComponent(int id, Component cp)
        {
            try
            {
                ...
                string updateString =
                    @"UPDATE Component
                        SET ComponentNumber= @Data2, SerieNr = @Data3, ComponentName = @Data4, ComponentInfo = @Data5, Category = @Data6, Datasheet = @Data7, Image = @Data8, OwnerID = @Data9, ReservationID = @Data10
                        WHERE ComponentID = @Data1";
                ...
                using (SqlCommand cmd = new SqlCommand(updateString, _conn))
                {
                    ...
                    cmd.Parameters["@Data10"].Value = cp.ReservationID != 0 ? cp.ReservationID : resIdParam;
        ...
        public bool ComponentReservation(string cName, int studienumber)
        {
            ...
            {
                string owner = Convert.ToString(comp.OwnerID);
                string reservation = Convert.ToString(comp.ReservationID);
                if (owner.Contains("0") && reservation.Contains("0"))
                {
                    comp.ReservationID = studienumber;
                    CurrentComponent = comp;
                    UpdateComponent(comp.ComponentID, CurrentComponent);
                    return true;
                }
            }
            return false;
