...
namespace Web.Models
{
    public class ComponentDataUtil
    {
        ...
        public ComponentDataUtil()
        {
            ...
            //_conn = new SqlConnection(@"Data Source=(localdb)\Projects;Initial Catalog=Database;Integrated Security=True");
        ...
        public bool CreateComponent(Component cp)
        {
            try
            {
                ...
                string insertStringParam =
                    @"INSERT INTO [Component] (ComponentID, ComponentNumber, SerieNr, ComponentName, ComponentInfo, Category, Datasheet, Image, OwnerID, LoanInformation)
                ...
                using (SqlCommand cmd = new SqlCommand(insertStringParam, _conn))
                {
                    ...
                    cmd.Parameters["@Data2"].Value = cp.ComponentNumber;
                    cmd.Parameters["@Data3"].Value = cp.SerieNr;
                    cmd.Parameters["@Data4"].Value = cp.ComponentName;
                    // Hvis parameterne er null, så brug DBNULL.Value.
                    cmd.Parameters["@Data5"].Value = cp.ComponentInfo ?? componentInfoParam;
                    cmd.Parameters["@Data6"].Value = cp.Category ?? categoryParam;
                    cmd.Parameters["@Data7"].Value = cp.Datasheet ?? datasheetParam;
                    cmd.Parameters["@Data8"].Value = cp.Image ?? imageParam;
                    cmd.Parameters["@Data9"].Value = cp.OwnerID != 0 ? cp.OwnerID : ownerIdParam;
                    cmd.Parameters["@Data10"].Value = cp.LoanInformation != 0 ? cp.LoanInformation : loanInformationParam;
            ...
            try
            {
                foreach (var comp in GetAllComponents())
                {
                    foreach (var keyword in keywords)
                    {
                        ...
                        if (comp.ComponentInfo.Contains(key))
                            compList.Add(comp);
                    }
                }
        ...
        public bool UpdateComponent(int id, Component cp)
        {
            try
            {
                ...
                string updateString =
                    @"UPDATE Component
                        SET ComponentNumber= @Data2, SerieNr = @Data3, ComponentName = @Data4, ComponentInfo = @Data5, Category = @Data6, Datasheet = @Data7, Image = @Data8, OwnerID = @Data9, LoanInformation = @Data10 
                        WHERE ComponentID = @Data1";
